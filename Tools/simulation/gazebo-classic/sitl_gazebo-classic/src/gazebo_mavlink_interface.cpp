...
void GazeboMavlinkInterface::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  getSdfParam<std::string>(_sdf, "reactionWheelRpmSubTopic", reaction_wheel_rpm_sub_topic_,
      reaction_wheel_rpm_sub_topic_);
  ...
  reaction_wheel_rpm_sub_ = node_handle_->Subscribe("~/" + model_->GetName() + reaction_wheel_rpm_sub_topic_,
      &GazeboMavlinkInterface::ReactionWheelRpmCallback, this);
...
void GazeboMavlinkInterface::ReactionWheelRpmCallback(FloatPtr& rpm_msg) {
  if (!std::isfinite(rpm_msg->data())) {
    return;
  }

  mavlink_message_t msg;
  mavlink_msg_raw_rpm_pack_chan(1, 200, MAVLINK_COMM_0, &msg, 0, rpm_msg->data());
  mavlink_interface_->send_mavlink_message(&msg);
}
